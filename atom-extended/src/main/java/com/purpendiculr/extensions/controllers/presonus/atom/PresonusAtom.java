...
public class PresonusAtom extends ControllerExtension {
	...
	public PresonusAtom(final PresonusAtomExtendedDefinition definition, final ControllerHost host) {
...
//		mCursorDevice = mCursorTrack.createCursorDevice();
	...
	private void initBaseLayer() {
		...
		mBaseLayer.bindToggle(mPlayLoopButton, () -> {
			...
			} else if (mTransport.isPlaying().get()) {
				mTransport.stop();
			} else {
				mTransport.play();
			}
		...
		mBaseLayer.bindToggle(mDownButton, () -> {
			mCursorTrack.selectNext();
			mCursorTrack.makeVisibleInArranger();
		}, mCursorTrack.hasNext());
	...
	private void initLauncherClipsLayer() {
		for (int i = 0; i < 16; i++) {
			...
			mLauncherClipsLayer.bind(() -> slot.hasContent().get() ? getClipColor(slot) : (Color) null, padButton);
		}
	}
	...
	private int getNumStepPages() {
		return (int) Math
				.ceil((mCursorClip.getLoopStart().get() + mCursorClip.getLoopLength().get()) / (16.0 * getStepSize()));
	}
	...
	private HardwareButton mShiftButton, mUpButton, mDownButton, mLeftButton, mRightButton, mSelectButton, mZoomButton,
			...
			mEditorButton, mNudgeQuantizeButton, mShowHideButton, mPresetPadSelectButton, mBankButton, mFullLevelButton,
			mNoteRepeatButton;
